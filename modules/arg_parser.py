...
def get_args():
    ...
    if args.oligo is None:
        print(
            "ERROR: the definiton for each oligomer must be specified. System exiting..."
        )
        sys.exit()

    if (
        args.L is None
        and args.seq is None
        and ".af2h" not in args.select_positions
    ):
        print(
            "ERROR: either seed sequence(s) or length(s) must specified. System exiting..."
        )
        sys.exit()

    if np.any(
        [
            (lambda x: True if x not in ["+", "-"] else False)(d[-1])
            for d in args.oligo.split(",")
        ]
    ):
        print(
            "ERROR: the type of design (positive [+] or negative [-]) must be specified for each oligomer. System existing..."
        )
        sys.exit()

    # Warnings.
    if (args.L is not None) and (
        args.seq is not None or ".af2h" in args.select_positions
    ):
        print(
            "WARNING: Both user-defined sequence(s) and length(s) were provided. Are you sure of what you are doing? The simulation will continue assuming you wanted to use the provided sequence(s) as seed(s)."
        )
    ...
    args.unique_protomers = sorted(
        set(args.oligo.replace(",", "").replace("+", "-").replace("-", ""))
    )
    ...
    if args.loss_weights != None:
        ...
        for curr_loss_weight in args.loss_weights.strip(",").split(","):
            if "-" in curr_loss_weight:
                ...
                print(
                    "loss weight ramping NOT IMPLMENTED YET. System exiting..."
                )
                sys.exit()
                loss_weights.append(
                    [float(i) for i in curr_loss_weight.split("-")]
                )
                pass
            else:
    ...
    if args.seq != None:
        ...
        with open(args.select_positions.split("::")[0], "r") as f:
            lines = list(
                line for line in (l.strip() for l in f) if line
            )  # strip empty lines.
            seq_prob = {}
            for entry in np.reshape(lines, (-1, 3)):
                ...
                ajd_freq = (
                    freq / freq.sum()
                )  # re-adjust frequencies to sum to 1 across the length of each protomer.
                seq_prob[entry[0][1:]] = {"seq": entry[1], "prob": ajd_freq}

            for (
                proto
            ) in (
                args.unique_protomers
            ):  # complete with empty entries in case the user did not specify all protomers -- these will be replace by randomly generated sequences at initalisation.
                if proto not in list(seq_prob.keys()):
                    seq_prob[proto] = {"seq": "", "prob": ""}

        args.proto_sequences = [
            seq_prob[proto]["seq"] for proto in args.unique_protomers
        ]
        args.position_weights = [
            list(seq_prob[proto]["prob"]) for proto in args.unique_protomers
        ]
    ...
    if args.L is None:
        ...
        args.proto_Ls = [
            int(length) if length != "" else 0 for length in args.L.split(",")
        ]
    ...
    if args.seq is None:
        if len(args.unique_protomers) != len(args.proto_Ls):
            print(
                "ERROR: the number of unique protomers and the number of specified lengths must match. System exiting..."
            )
            sys.exit()
    ...
    if args.L is None:
        if len(args.unique_protomers) != len(args.proto_Ls):
            print(
                "ERROR: the number of unique protomers and the number of specified sequences must match. System exiting..."
            )
            sys.exit()

    if "tmalign" in args.loss and args.template is None:
        print(
            "ERROR: tmalign loss require a --template [PDBFILE]. System exiting..."
        )
        sys.exit()
